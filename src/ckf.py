...
class CKF:
    ...
    def generate_cubature_points(self):
        try:
            ...
            # Fallback to diagonal if Cholesky fails (e.g. numerical issues)
            S = np.diag(np.sqrt(np.maximum(np.diag(self.P), 1e-12)))

        scale = np.sqrt(self.n)
        xi = np.zeros((2 * self.n, self.n))
        
        # Points are scale * [I, -I]
        for i in range(self.n):
            xi[i] = scale * S[:, i]
            xi[self.n + i] = -scale * S[:, i]
            
        return xi

    def predict(self, omega_meas, dt):
        # Generate Cubature Points (Error State)
        xi = self.generate_cubature_points()
        ...
        for i in range(2 * self.n):
            # Error components (using MRPs for attitude error)
            d_mrp = xi[i, :3]
            d_beta = xi[i, 3:]
        ...
        # Compute Mean State
        # Bias mean: linear
        mean_beta = np.sum(self.W * propagated_states[:, 4:], axis=0)
        
        # Quaternion mean: gradient descent
        ...
        for i in range(2 * self.n):
            ...
            P_pred += self.W * np.outer(delta_x, delta_x)
            
        self.P = P_pred + self.Q
        
    def update(self, z_meas, z_ref):
        # New cubature points from P_pred
        xi = self.generate_cubature_points()
        ...
        for i in range(2 * self.n):
            ...
            Z_points.append(z_pred_i)
            
        Z_points = np.array(Z_points)
        
        # Mean measurement
        z_mean = np.sum(self.W * Z_points, axis=0)
        ...
        for i in range(2 * self.n):
            z_diff = Z_points[i] - z_mean
            x_diff = xi[i] 
            
            P_zz += self.W * np.outer(z_diff, z_diff)
            P_xz += self.W * np.outer(x_diff, z_diff)
